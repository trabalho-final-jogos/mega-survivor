...
bool Renderer::Initialize(float width, float height) {
  ...
  mOrthoProjection = Matrix4::CreateOrtho(
      0.0f, Game::VIRTUAL_WIDTH, Game::VIRTUAL_HEIGHT, 0.0f, -1.0f, 1.0f);
...
void Renderer::Draw(RendererMode mode,
  ...
  if (mode == RendererMode::LINES) {
    glDrawElements(GL_LINE_LOOP, vertices->GetNumIndices(), GL_UNSIGNED_INT,
                   nullptr);
...
void Renderer::DrawTexture(const Vector2& position,
  ...
  Draw(RendererMode::TRIANGLES, model, cameraPos, mSpriteVerts, color, texture,
       textureRect, textureFactor);
}
...
void Renderer::CreateSpriteVerts() {
  float vertices[] = {-0.5f, 0.5f,  0.0f, 1.0f, 0.5f,  0.5f,  1.0f, 1.0f,
                      0.5f,  -0.5f, 1.0f, 0.0f, -0.5f, -0.5f, 0.0f, 0.0f};
