    ...
    if (mCurrentWaveIndex < mWaves.size())
    {
        ...
        if (gameTime >= currentWave.endTime)
        {
            ...
            if (mCurrentWaveIndex < mWaves.size())
            {
                ...
                // Se for Morcego, TRAVA um lado aleatório para criar o efeito de "travessia"
                if (nextWave.type == EnemyType::Bat)
                {
                    mCurrentWaveSide = rand() % 4;
                    SDL_Log("!!! ENXAME DE MORCEGOS VINDO DO LADO %d !!!", mCurrentWaveSide);
                }
                else
                {
...
void Spawner::SpawnEnemy()
{
    ...
    if (mCurrentWaveIndex < mWaves.size()) wave = &mWaves[mCurrentWaveIndex];
    else if (!mWaves.empty()) wave = &mWaves.back();
    else return;
    ...
    else if (wave->type == EnemyType::Bat)
    {
        ...
        // Injeta Stats (Vida)
        // (Nota: Bat usa velocidade no SetFixedVelocity, mas podemos setar vida aqui)
        // bat->SetStats(wave->enemyHealth);
        ...
        float randomOffset = (static_cast<float>(rand()) / RAND_MAX) * 100.0f - 50.0f;

        Vector2 target;
        if (side == 0 || side == 1) target = Vector2(spawnPos.x + randomOffset, playerPos.y); // Cruza verticalmente
        else target = Vector2(playerPos.x, spawnPos.y + randomOffset); // Cruza horizontalmente
