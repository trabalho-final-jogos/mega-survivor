...
void BoomerangProjectile::Awake(Actor* owner, const Vector2 &position, float rotation,
                              float lifetime)
{
    // 1. Chama a base (define mShooter, Posição, Vida, Estado, Ativa Colisor/Desenho)
...
void BoomerangProjectile::OnUpdate(float deltaTime)
{
    ...
    float x = (one_minus_t * one_minus_t * p0.x) + (2.0f * one_minus_t * t * p1.x) + (t * t * p2.x);
    float y = (one_minus_t * one_minus_t * p0.y) + (2.0f * one_minus_t * t * p1.y) + (t * t * p2.y);
...
void BoomerangProjectile::OnHorizontalCollision(const float minOverlap, AABBColliderComponent* other)
{
    ...
    else if (layer == ColliderLayer::Player && mState == BoomerangState::Returning)
    {
...
void BoomerangProjectile::OnVerticalCollision(const float minOverlap, AABBColliderComponent* other)
{
