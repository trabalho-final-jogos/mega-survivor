...
Bullet::Bullet(Game* game, int width, int height)
    : Projectile(game, width, height) // Chama o construtor da base (Projectile)
{
    if (mDrawComponent) {
...
void Bullet::OnHorizontalCollision(const float minOverlap, AABBColliderComponent* other)
{
    ...
    if (otherLayer == ColliderLayer::Blocks || otherLayer == ColliderLayer::Enemy)
    {
        // Se for um inimigo, causa dano/mata
...
void Bullet::OnVerticalCollision(const float minOverlap, AABBColliderComponent* other)
{
