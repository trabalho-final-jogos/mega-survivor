...
MainGun::MainGun(Actor* owner, int updateOrder)
  ...
  for (int i = 0; i < POOL_SIZE; i++) {
    ...
    auto bullet =
        new Bullet(mOwner->GetGame(), PARTICLE_WIDTH, PARTICLE_HEIGHT);
...
void MainGun::FireShot() {
  ...
  // Pega a posição de disparo (o jogador pode ter-se movido desde o início da
  // rajada)
  ...
  if (p) {
    ...
    p->Awake(mOwner, playerPos, mOwner->GetRotation(), mProjectileLifetime,
             mBurstVelocity, mDamage, mAreaScale);

    GetGame()->GetAudioSystem()->PlaySound("click.wav");
