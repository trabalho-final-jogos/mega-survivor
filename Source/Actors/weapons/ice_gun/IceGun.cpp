    ...
    , mCooldownTimer(0.0f)
{
    ...
    {
        auto bullet = new IceProjectile(mOwner->GetGame(), PARTICLE_WIDTH, PARTICLE_HEIGHT);
...
void IceGun::FireShot()
{
    ...
    if (centerDirection.LengthSq() == 0.0f) { centerDirection = Vector2(1.0f, 0.0f); }
    ...
    float angleIncrement = (NUM_PROJECTILES > 1) ? (totalSpreadRad / (NUM_PROJECTILES - 1)) : 0.0f;
    float currentAngle = (NUM_PROJECTILES > 1) ? (-totalSpreadRad / 2.0f) : 0.0f; // Começa em -15 graus
    ...
    for (int i = 0; i < NUM_PROJECTILES; i++)
    {
        ...
        if (!p) { break; } // Parar se o pool estiver vazio
