...
LaserProjectile::LaserProjectile(Game* game, int width, int height)
    : Projectile(game,
                 width,
                 height)  // Chama a base (que carrega o spritesheet)
      ,
      mBouncesLeft(0),
...
void LaserProjectile::Awake(Actor* owner,
  ...
  Projectile::Awake(owner, position, rotation, lifetime, velocity, damage,
                    areaScale);
  GetGame()->GetAudioSystem()->PlaySound("shot3.wav");
  if (mDrawComponent) {
...
void LaserProjectile::OnUpdate(float deltaTime) {
  // 1. Chama o Update da base (que verifica o LifeTime e chama Kill() se
  // expirar)
...
void LaserProjectile::OnHorizontalCollision(const float minOverlap,
                                            AABBColliderComponent* other) {
  ColliderLayer otherLayer = other->GetLayer();

  if (otherLayer == ColliderLayer::Enemy) {
    Actor* enemyActor = other->GetOwner();
    bool alreadyHit = std::find(mEnemiesHit.begin(), mEnemiesHit.end(),
                                enemyActor) != mEnemiesHit.end();
...
void LaserProjectile::OnVerticalCollision(const float minOverlap,
                                          AABBColliderComponent* other) {
