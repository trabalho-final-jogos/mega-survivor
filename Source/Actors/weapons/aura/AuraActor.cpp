    ...
    , mHitTimer(0.0f)
{
    ...
    mCollider = new AABBColliderComponent(this, 0, 0, 
                                          Game::TILE_SIZE * 2, Game::TILE_SIZE * 2, // Tamanho base
...
void AuraActor::ApplyHit(Actor* enemy)
{
    ...
    if (mHitTimer > 0.0f) { return; } // Ainda em cooldown

    // Verifica se já atingimos este inimigo neste tick
    if (std::find(mEnemiesHitThisTick.begin(), mEnemiesHitThisTick.end(), enemy) 
        == mEnemiesHitThisTick.end())
    {
...
void AuraActor::OnHorizontalCollision(const float minOverlap, AABBColliderComponent* other)
{
...
void AuraActor::OnVerticalCollision(const float minOverlap, AABBColliderComponent* other)
{
    if (other->GetLayer() == ColliderLayer::Enemy)
    {
        ApplyHit(other->GetOwner());
